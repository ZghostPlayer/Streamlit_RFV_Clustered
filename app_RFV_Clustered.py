...
def to_excel(df):
    ...
    writer.close()  # <- Atualização necessária
...
def main():
    ...
    if (data_file_1 is not None):
        ...
        centroides = pd.DataFrame(kmeans.cluster_centers_, columns=['Recencia', 'Frequencia', 'Valor'])
        st.write(centroides)  

                # Visualizando os clusters (Recência x Frequência)
        fig, ax = plt.subplots()
        sns.scatterplot(x=df_RFV['Recencia'], y=df_RFV['Frequencia'], hue=df_RFV['Cluster'], palette='viridis', ax=ax)
        ax.set_title('Clusters de Clientes (Recência x Frequência)')
        st.pyplot(fig)

        # Visualizando os clusters (Frequência x Valor)
        fig, ax = plt.subplots()
        sns.scatterplot(x=df_RFV['Frequencia'], y=df_RFV['Valor'], hue=df_RFV['Cluster'], palette='viridis', ax=ax)
        ax.set_title('Clusters de Clientes (Frequência x Valor)')
        st.pyplot(fig)
